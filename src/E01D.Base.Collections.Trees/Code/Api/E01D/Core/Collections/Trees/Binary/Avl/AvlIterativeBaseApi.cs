...
{
    public abstract class AvlIterativeBaseApi<TNode, TKey> : AvlApi_I<TNode, TKey>
    {
        ...
        public TNode Find(TNode node, TKey key, Func<TKey, TKey, long> comparer)
        {
            if (IsNull(node)) return NullNode();

            var currentNode = node;

            while (currentNode != null)
            {
                var compareValue = CompareKeys(GetKey(currentNode), key);

                if (compareValue == 0) return currentNode;

                currentNode = compareValue > 0 ? GetRight(currentNode) : GetLeft(currentNode);
            }
        ...
        {
            Debug.WriteLine("Rotate Left Right");
        ...
        {
            Debug.WriteLine("Rotate Right Left");
        ...
        public TNode Remove(TNode root, TKey key)
        {
            ...
            var nodeToRemove = Find(root, key, out stack);
        ...
        public TNode CreateNode(TNode existingNode, int balanceFactor)
        {
            var newNode = CreateNodeInternal(existingNode, balanceFactor);

            SetValue(existingNode, newNode);

            return newNode;
        }

        public TNode DuplicateNode(TNode existingNode, TNode left, TNode right)
        {
            var newNode = DuplicateNodeInternal(existingNode, left, right);
        ...
        public TNode DuplicateNodeFromNode(TNode existingNode)
        {
            var newNode = DuplicateNodeFromNodeInternal(existingNode);
        ...
        public TNode SetLeft(TNode existing, TNode newLeft, int newBalanceFactor)
        {
            var newNode = SetLeftInternal(existing, newLeft, newBalanceFactor);

            SetValue(existing, newNode);

            return newNode;
        }


        public TNode SetRight(TNode existing, TNode newRight, int newBalanceFactor)
        {
            var newNode = SetRightInternal(existing, newRight, newBalanceFactor);
        ...
        public TNode SetLeft(TNode existing, TNode newLeft)
        {
            var newNode = SetLeftInternal(existing, newLeft);
        ...
        public TNode SetRight(TNode existing, TNode newRight)
        {
            var newNode = SetRightInternal(existing, newRight);
        ...
        public TNode SetBalanceFactor(TNode existing, int newFactor)
        {
            var newNode = SetBalanceFactorInternal(existing, newFactor);
        ...
        public abstract TNode CreateNodeInternal(TNode newNode, int balanceFactor);

        public abstract TNode DuplicateNodeInternal(TNode node, TNode left, TNode right);

        public abstract TNode DuplicateNodeFromNodeInternal(TNode source);


        public abstract AvlIterativeBaseApi<TNode, TKey> GetApi();
        ...
        public abstract TNode SetBalanceFactorInternal(TNode existing, int newFactor);


        public abstract TNode SetLeftInternal(TNode existing, TNode newLeft);

        public abstract TNode SetLeftInternal(TNode existing, TNode newLeft, int newBalanceFactor);

        public abstract TNode SetRightInternal(TNode existing, TNode newRight);

        public abstract TNode SetRightInternal(TNode existing, TNode newRight, int newBalanceFactor);

        public abstract void SetValue(TNode existing, TNode newNode);
