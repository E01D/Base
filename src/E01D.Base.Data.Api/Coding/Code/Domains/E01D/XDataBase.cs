...
namespace Root.Coding.Code.Domains.E01D
{
    public static class XDataBase
    {
        ...
        {
            DataConnectionApi_I provider = null;//XApis.Api.GetApi<DataConnectionApi_I, T>();

            if (provider == null)
            {
                var context = XContextual.GetGlobal<DataGlobalContext_I>();

                provider = (DataConnectionApi_I) context.DefaultDataConnectionProvider;//XApis.Api.GetApi<DataConnectionApi_I>();
            }
