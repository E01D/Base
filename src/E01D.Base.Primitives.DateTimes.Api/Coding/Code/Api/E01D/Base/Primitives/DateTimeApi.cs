...
using System.IO;
using System.Xml;
using Root.Coding.Code.Api.E01D.Base.Primitives.DateTimes;
using Root.Coding.Code.Domains.E01D;
using Root.Coding.Code.Enums.E01D.Core.DateTimes;
using Root.Coding.Code.Enums.E01D.Parsing;
using Root.Coding.Code.Exts.E01D.Base.Primitives.DateTimes;
...
namespace Root.Coding.Code.Api.E01D.Base.Primitives
{
    ...
    {
        public DateTimeParseApi Parsing { get; set; } = new DateTimeParseApi();
        ...
        public XmlDateTimeSerializationMode ToSerializationMode(DateTimeKind kind)
        {
            switch (kind)
            {
                ...
                default:
                    throw XExceptions.Argument.OutOfRange.UnexpectedDateTimeKindValue(nameof(kind), kind);
            }
        }
#else
        ...
        public DateTime EnsureDateTime(DateTime value, DateTimeZoneHandling timeZone)
        {
            switch (timeZone)
            {
        ...
        public DateTime SwitchToLocalTime(DateTime value)
        {
            switch (value.Kind)
            {
                case DateTimeKind.Unspecified:
        ...
        public DateTime SwitchToUtcTime(DateTime value)
        {
            switch (value.Kind)
            {
                case DateTimeKind.Unspecified:
        ...
        public long ToUniversalTicks(DateTime dateTime)
        {
        ...
        public long ToUniversalTicks(DateTime dateTime, TimeSpan offset)
        {
        ...
        public long ConvertDateTimeToJavaScriptTicks(DateTime dateTime, TimeSpan offset)
        {
        ...
        public long ConvertDateTimeToJavaScriptTicks(DateTime dateTime)
        {
            return ConvertDateTimeToJavaScriptTicks(dateTime, true);
        }

        public long ConvertDateTimeToJavaScriptTicks(DateTime dateTime, bool convertToUtc)
        {
        ...
        public long UniversialTicksToJavaScriptTicks(long universialTicks)
        {
        ...
        public DateTime ConvertJavaScriptTicksToDateTime(long javaScriptTicks)
        {
        ...
        public bool TryParseDateTime(StringReference s, DateTimeZoneHandling dateTimeZoneHandling, string dateFormatString, CultureInfo culture, out DateTime dt)
        {
            if (s.Length > 0)
            {
                int i = s.StartIndex;
                ...
                else if (s.Length >= 19 && s.Length <= 40 && char.IsDigit(s.Get(i)) && s.Get(i + 10) == 'T')
                {
        ...
        public bool TryParseDateTimeOffset(StringReference s, string dateFormatString, CultureInfo culture, out DateTimeOffset dt)
        {
            if (s.Length > 0)
            {
                int i = s.StartIndex;
        ...
        public bool TryParseDateTimeOffset(string s, string dateFormatString, CultureInfo culture, out DateTimeOffset dt)
        {
            if (s.Length > 0)
            {
                if (s[0] == '/')
                {
                    if (s.Length >= 9 && s.StartsWith("/Date(", StringComparison.Ordinal) && s.EndsWith(")/", StringComparison.Ordinal))
                    {
                        if (TryParseDateTimeOffsetMicrosoft(XStrings.StringReference(s.ToCharArray(), 0, s.Length), out dt))
                        {
                            return true;
                        }
                    }
                ...
                else if (s.Length >= 19 && s.Length <= 40 && char.IsDigit(s[0]) && s[10] == 'T')
                {
                    ...
                    {
                        if (TryParseDateTimeOffsetIso(XStrings.StringReference(s.ToCharArray(), 0, s.Length), out dt))
                        {
                            return true;
                        }
                    }
        ...
        public bool TryParseMicrosoftDate(StringReference text, out long ticks, out TimeSpan offset, out DateTimeKind kind)
        {
            ...
            return (XConvert.Int64TryParse(text.Chars, 6 + text.StartIndex, index - 6, out ticks) == ParseResult.Success);
        }

        public bool TryParseDateTimeMicrosoft(StringReference text, DateTimeZoneHandling dateTimeZoneHandling, out DateTime dt)
        {
            long ticks;
            TimeSpan offset;
            DateTimeKind kind;
        ...
        public bool TryParseDateTimeExact(string text, DateTimeZoneHandling dateTimeZoneHandling, string dateFormatString, CultureInfo culture, out DateTime dt)
        {
            DateTime temp;
        ...
        public bool TryParseDateTimeOffsetMicrosoft(StringReference text, out DateTimeOffset dt)
        {
            long ticks;
            TimeSpan offset;
            DateTimeKind kind;
        ...
        public bool TryParseDateTimeOffsetExact(string text, string dateFormatString, CultureInfo culture, out DateTimeOffset dt)
        {
            DateTimeOffset temp;
        ...
        public bool TryReadOffset(StringReference offsetText, int startIndex, out TimeSpan offset)
        {
            bool negative = (offsetText.Get(startIndex) == '-');

            int hours;
            if (XConvert.Int32TryParse(offsetText.Chars, startIndex + 1, 2, out hours) != ParseResult.Success)
            {
            ...
            {
                if (XConvert.Int32TryParse(offsetText.Chars, startIndex + 3, 2, out minutes) != ParseResult.Success)
                {
                    offset = default(TimeSpan);
                    return false;
                }
        ...
        #region Write
        public void WriteDateTimeString(TextWriter writer, DateTime value, DateFormatHandling format, string formatString, CultureInfo culture)
        {
        ...
        public int WriteDateTimeString(char[] chars, int start, DateTime value, TimeSpan? offset, DateTimeKind kind, DateFormatHandling format)
        {
            int pos = start;
        ...
        public int WriteDefaultIsoDate(char[] chars, int start, DateTime dt)
        {
            int length = 19;
        ...
        public void CopyIntToCharArray(char[] chars, int start, int value, int digits)
        {
            while (digits-- != 0)
            {
        ...
        public void WriteDateTimeOffsetString(TextWriter writer, DateTimeOffset value, DateFormatHandling format, string formatString, CultureInfo culture)
        {
            if (string.IsNullOrEmpty(formatString))
            {
                char[] chars = new char[64];
        ...
        public void GetDateValues(DateTime td, out int year, out int month, out int day)
        {
            long ticks = td.Ticks;
