...
namespace Root.Code.Api.E01D.Core.IO.Codecs.LittleEndian
{
    public class StringApi:Codec_I<string>
    {
        ...
        public void WriteUTF16String(byte[] bytes, int offset, string value)
        {
            if (value == null)
            {
                XCodecs.Api.LittleEndian.WriteInt32(bytes, offset, -1);
            }
            else
            {
                XCodecs.Api.LittleEndian.WriteInt32(bytes, offset, value.Length);

                for (var i = 0; i < value.Length; i++)
                {
                    XCodecs.Api.LittleEndian.WriteChar(bytes, 4 + (2 * i), value[i]);
                }
            }
        }
        ...
        {
            var length = XCodecs.Api.LittleEndian.ReadInt32(bytes, offset);
            ...
            {
                x[i] = XCodecs.Api.LittleEndian.ReadChar(bytes, 4 + (2 * i));
            }
